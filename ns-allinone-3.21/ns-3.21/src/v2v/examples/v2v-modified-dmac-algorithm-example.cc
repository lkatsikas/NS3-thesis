...
using namespace ns3;
using namespace std;
NS_LOG_COMPONENT_DEFINE("V2vModifiedDMACExample");
...
void printNumberofClusterMembers(NodeContainer ueNodes){
    ...
    for (uint32_t u = 0; u < ueNodes.GetN(); ++u) {
        ...
        Ptr<V2vModifiedDMACAlgorithmClient> tmp = temp->GetObject<V2vModifiedDMACAlgorithmClient>();
    ...
    NS_LOG_UNCOND("Average number of members is: " << sum/chs);
}
...
int main(int argc, char *argv[]) {
    ...
    NS_LOG_UNCOND("/--------------------------------------------------------------------------------\\");
    NS_LOG_UNCOND(" - Modified DMAC Algorithm for VANETs [Example] -> Cluster vehicles communication");
    NS_LOG_UNCOND("\\--------------------------------------------------------------------------------/");
    /*----------------------------------------------------------------------*/
    ...
    for (uint32_t u = 0; u < ueNodes.GetN(); ++u) {
        ...
        V2vModifiedDMACAlgorithmHelper ueClient("ns3::UdpSocketFactory", Address(InetSocketAddress(Ipv4Address::GetBroadcast(), controlPort)),
